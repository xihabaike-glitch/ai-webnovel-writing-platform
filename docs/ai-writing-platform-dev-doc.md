...
1. `/docs`：开发文档总览。对齐主控闸门 路线、8 个核心平台、30+ GitHub 参考案例、AI 模型接口和大树写作流程。
...
主控闸门 的批量生产闸门只允许三种放量结论，不能各自发明放量口径。

1. 允许小步加码：样本、复查、质量、成本和失败回收都过线；可以继续小批执行，但仍要保留下一批真实成功率、质量、成本和失败回执。
2. 继续观察：样本能跑通但证据还薄，或恢复小批只稳定 1 次，或首日扩展只证明单个样本过线；只能继续小样本，不退出恢复模式，不扩大到普通批量。
3. 禁止放大：失败样本、低质量、成本异常、模型岗位缺失、路线未复检或恢复小批跌线；先进入失败修复、回滚观察或模型岗位修复，不允许继续推荐批次。

这套三态已经在四个页面同步展示：`/tasks` 的推荐批次安全阀和恢复小批卡、`/gate` 的批量打法效果复盘、`/projects/[projectId]` 的最近 AI 批次回流、`/failures` 的失败修复恢复小批回流。

适用范围包括推荐批次安全阀、恢复小批、首日扩展、批量打法复盘和项目总控最近批次。新增任何批量生产入口时，必须复用同一套三态字段：`scaleDecisionLabel`、`scaleDecisionTone`、`scaleDecisionDetail`。

### 0.4.2 顶部总控统一闭环看板

项目详情页的生产决策卡已经增加顶部总控统一闭环看板。它把批量健康、AI 写审改、模型路线三条生产线合并到同一个 `productionClosure` 状态数组里，让作者只看一个总控入口就能判断能否继续生产。
...
这条路线用于验一部真实作品样本，不跳过人工采用，不用新增平台掩盖流水线问题。

1. 开书与大树骨架：在 `/projects` 检查目标平台、篇幅、开头钩子、结尾承诺、主干和基础土壤。缺目标平台、开头钩子或结尾承诺时停在作品工作台，不进入模型生成。
2. 首章样本生成：首章样本必须进入候选稿、审稿和二改链路，不能直接覆盖正文。样本不过钩子、爽点、人物弧光和平台适配时，只允许二改或重写，不允许批量生产。
3. 任务与派单回执：在 `/dispatch` 检查模型执行角色、输入、输出、任务回执、人工验收和下一步任务。回执证据太薄或没有人工采用时，不进入总闸门放大。
4. 总闸门放大检查：在 `/gate` 检查样本、复查、成本、质量、失败率和恢复证据。缺少样本、复查或失败修复证据时，不允许批量。
5. 失败修复与恢复观察：在 `/failures` 按模型配置、提示词上下文、样本重试和人工复盘分泳道处理。模型配置或上下文失败未修复时暂停批量。
6. 发布包与平台复盘：回到 `/projects` 检查发布包、导出、平台策略、版本基线和效果复盘。8 个核心平台都要有写作抓手、投稿抓手、复盘指标和版本证据。

### 0.7 流水线验收回执模板

每一步都要填证据、通过信号、退回原因和负责人确认。证据不够就退回修复，不允许用口头感觉放行。

最终结论只能选三种：通过、退回修复、暂停批量。

1. 开书与大树骨架回执：记录作品名、目标平台、篇幅、开头钩子、结尾承诺、主干和土壤是否齐备。缺目标平台、开头钩子或结尾承诺，退回作品工作台。
2. 首章样本生成回执：记录首章样本、审稿问题、二改候选和人工采用结论。候选稿直接覆盖正文、没有人工采用或样本质量不足仍想批量，退回修复。
3. 任务与派单回执：记录模型执行角色、输入、输出、任务回执、人工验收和下一步入口。回执证据太薄、没有人工验收或没有下一步，退回派单中心。
4. 总闸门放大回执：记录样本、复查、成本、质量、失败率和是否允许小批量。缺少复查、失败率过高或没有恢复证据，暂停批量。
5. 失败修复回执：记录失败原因、修复泳道、重试样本、恢复观察和是否仍需暂停批量。模型配置未修复、上下文失败未修复或未恢复失败仍存在，暂停批量。
6. 发布包与平台复盘回执：记录平台包、样章、标签、卖点、版本基线和反馈复盘。发布包缺样章、平台卖点不清或反馈记录缺失，退回发布修复。

#### 0.7.1 真实作品流水线终检清单

终检入口：`/gate?focus=action-recheck&source=real-sample-receipt#gate-focus-notice`。页面按钮文案：查看真实样本回执复检。

通过：
1. 开书证据、首章样本、审稿二改和人工采用都能互相追溯。
2. 发布包、版本基线、平台复盘指标已经回到作品页。
3. 总闸门复检后剩余卡点减少，且没有未恢复失败。

退回修复：
1. 任一环节缺少负责人确认、回执依据或下一步入口，退回对应工作区修复。
2. 首章钩子、平台卖点或前三章兑现不足，退回审稿二改或发布包修复。

暂停批量：
1. 缺人工采用、缺复查、失败率过高或恢复观察不足时暂停批量。
2. 模型岗位缺岗、成本失控或发布反馈缺失时暂停批量。

#### 0.7.2 真实作品样本运行手册

这张手册用于实跑一部作品样本，不用于新增平台，也不用于跳过人工采用。

| 步骤 | 负责人 | 样本动作 | 要抓的证据 | 退路 |
| --- | --- | --- | --- | --- |
| 开书与大树骨架 | 作者 + 作品工作台负责人 | 选一部真实作品开书，补齐目标平台、篇幅、开头钩子、结尾承诺、主干、分支和土壤。 | 保存作品页的大树骨架、平台选择、篇幅模板和可进入首章样本的截图或回执。 | 目标平台、开头钩子或结尾承诺缺失时，回到作品页补骨架，不进入模型生成。 |
| 首章样本生成 | 中文网文写手 + 长篇结构主编 | 只生成首章样本，走候选稿、审稿、二改和人工采用，不直接覆盖正式正文。 | 保存首章样本、审稿问题、二改候选、人工采用结论和未采用原因。 | 钩子、爽点、人物弧光或平台节奏不过线时，退回二改或重写，不进入批量。 |
| 任务与派单回执 | 派单负责人 + AI 编辑部角色 | 把首章、审稿、二改、发布包或失败修复生成正式派单，并要求角色、输入、输出齐全。 | 保存派单回执、执行模型、输入摘要、输出摘要、人工验收和下一步入口。 | 缺执行角色、输入输出或人工验收时，退回派单中心补回执，不送总闸门。 |
| 总闸门放大检查 | 产品验收闸门 | 把真实样本带回总闸门复检，只判断是否允许小样本继续，不直接宣布批量跑通。 | 保存样本、复查、成本、质量、失败率、恢复证据和总闸门剩余卡点变化。 | 缺复查、失败率过高或模型岗位缺岗时，暂停批量并转入对应修复入口。 |
| 失败修复与恢复观察 | 失败修复中心 | 按模型配置、提示词上下文、样本重试、人工复盘分泳道处理失败，再跑恢复小样本。 | 保存失败原因、修复泳道、重试样本、恢复观察记录和是否仍需暂停批量。 | 未恢复失败仍存在或恢复观察不足时，继续小样本观察，不扩大批量。 |
| 发布包与平台复盘 | 投稿包装编辑 + 反馈运营 | 为目标平台生成发布包，回收标题、简介、标签、样章、版本基线和真实反馈。 | 保存发布包、导出版本、平台卖点、样章、反馈记录、复盘指标和下一轮修订任务。 | 平台卖点、前三章兑现、标签或反馈记录缺失时，停在发布修复，不宣称流水线完成。 |

### 0.8 端到端可见验收口径地图

这些口径必须出现在产品页面里，不只停留在文档里。后续新增入口时，先判断它属于哪条回执链，再复用对应口径。

1. `/projects`：作品工作台和投稿资料面板承接开书、发布包与平台复盘验收口径。发布包与平台复盘验收口径固定检查平台包、样章、标签、卖点、版本基线、真实反馈和复盘指标。
2. `/tasks`：任务中心承接任务回执验收口径。每个推荐任务必须让作者看到执行角色、输入、输出、人工验收和下一步，不满足时不能把“已执行”当作“已放行”。
3. `/dispatch`：派单中心承接派单回执验收口径。完成派单前必须补执行角色、输入、输出、人工验收和下一步，回总闸门时才有可复检证据。
4. `/gate`：总闸门承接放量复检和派单证据总控。它读取任务与派单回执、失败修复回执和发布包证据；缺样本、复查、质量、成本、失败率或恢复证据时禁止放大。
5. `/failures`：失败修复中心承接失败修复回执验收口径。固定检查失败原因、修复泳道、重试样本、恢复观察和是否暂停批量；没有恢复观察时，只能小样本或暂停批量。

主控闸门 口径：页面上看不到的证据，不算证据；没有下一步的回执，不算闭环。

### 0.9 原始需求最终验收矩阵

最终验收不看“感觉差不多”，只看证据链接、验收状态、缺口和下一步。没有证据链接的要求，不算完成。

| 原始要求 | 验收状态 | 证据链接 | 缺口 | 下一步 |
|---|---|---|---|---|
| 30 个开源参考案例 | 已覆盖 | `/references#development-path` | 无参考数量缺口 | 只把案例转成写作、派单、模型和平台动作，不继续堆资料。 |
...
| 真实作品流水线 | 观察中 | `/gate?focus=action-recheck&source=final-delivery-receipt#pipeline-final-review` | 真实作品流水线已接入首章样本回执和最终交付正式放行卡，仍需用真实作品持续验收正式放行证据 | 回总闸门复检首章样本回执和真实流水线终检，再决定交付、修复或暂停批量。 |
...
6. 从 `/projects` 和 `/tasks` 回收发布包：导出、平台包、版本基线、真实反馈和复盘指标要能回到作品。

主控闸门 结论：不再把新增平台当作进度，不再把模型生成当作完成。只有一部作品能从开书、首章样本、人工采用、总闸门、发布包到复盘跑通，才算这轮产品开发真的站住。
