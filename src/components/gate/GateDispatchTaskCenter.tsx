...
export function GateDispatchTaskCenter({
  ...
  initialRoleIntent?: DispatchRoleIntent | null;
    ...
    <div className="grid gap-6">
      ...
      {initialRoleIntent ? (
        <section className="rounded-md border border-slate-200 bg-white p-4">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div>
              <div className="text-xs font-medium text-slate-500">角色任务草稿</div>
              <h2 className="mt-1 text-lg font-semibold text-slate-950">{initialRoleIntent.roleName}</h2>
              <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
                建议模型：{initialRoleIntent.modelOwner}。这张卡还没有落库，先按角色检查现有派单，确认材料齐了再创建正式任务。
              </p>
              <p className="mt-2 rounded-md bg-slate-50 p-2 text-xs leading-5 text-slate-600">
                验收：{initialRoleIntent.acceptance}
              </p>
            </div>
            <div className="flex shrink-0 flex-wrap gap-2 lg:justify-end">
              <button
              ...
              <Link
                className="rounded-md border border-slate-200 px-3 py-2 text-sm font-medium text-slate-800 hover:bg-slate-50"
                href={hrefWithGateReturn(initialRoleIntent.returnHref, gateReturnHref)}
              >
                回作品工作区
              </Link>
            </div>
