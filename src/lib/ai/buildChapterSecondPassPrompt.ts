...
export function buildChapterSecondPassPrompt(input: ChapterSecondPassPromptInput) {
  ...
  const hardRequirements = [
    "只输出二改后的正文。",
    "不要把故事改成梗概，必须写现场、动作、对话和结果。",
    "保留章节核心事件，不要另开新故事。",
    "开头必须比当前稿更快进入冲突。",
    "结尾必须扣住章末悬念，不能平收。",
    input.startTactic ? "优先执行首轮平台打法，尤其是开头动作和验证动作，不要把它写成注释。" : null,
    startExperiencePromptBlock ? "二改必须按开书经验执行摘要修正文：复制动作要进正文，不能踩的边界必须避开。" : null,
    input.projectContext ? "二改必须遵守项目上下文召回计划；不要改断人物弧光、世界规则、伏笔状态和历史章节承接。" : null,
  ...
  const userPrompt = [
    ...
    "",
    "硬性要求：",
    ...hardRequirements.map((item, index) => `${index + 1}. ${item}`),
  ].join("\n");
