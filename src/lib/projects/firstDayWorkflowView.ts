...
function watchSampleEvidenceGaps(completionEvidence: string) {
  const gaps: string[] = [];
  if (!/成功率\s*[：:]?\s*\d+%/u.test(completionEvidence)) gaps.push("成功率");
  if (!/质量分?\s*[：:]?\s*\d+/u.test(completionEvidence)) gaps.push("质量分");
  if (!/失败样本\s*[：:]?\s*(?:0|无|\d+)/u.test(completionEvidence)) gaps.push("失败样本");
  if (!/放量结论\s*[：:][^\n]*(?:通过|允许|可以恢复|可恢复|恢复后续)/u.test(completionEvidence)) gaps.push("放量结论");
  if (/放量结论\s*[：:][^\n]*(?:未通过|继续(?:停留)?观察|暂不放量|不允许|不能放量)/u.test(completionEvidence)) {
    gaps.push("正向放量结论");
  }
  return gaps;
}
...
export function validateFirstDayDispatchCompletionEvidence(input: FirstDayDispatchCompletionValidationInput): FirstDayDispatchCompletionValidation {
  ...
  const watchMetricGaps = level === "watch" ? watchSampleEvidenceGaps(trimmedEvidence) : [];
  if (watchMetricGaps.length > 0) {
    return {
      valid: false,
      level,
      error: `小样本验证派单必须补齐结构化证据：${watchMetricGaps.join("、")}。`,
    };
  }
...
function watchSampleCompletionCleared(completionEvidence: string) {
  return /小样本|通过线|放量结论/u.test(completionEvidence)
    ...
    && watchSampleEvidenceGaps(completionEvidence).length === 0
...
function handoffEvidenceCompletionLines(evidence: string[] = []) {
...
function withHandoffCompletion(base: string, acceptanceCriteria: string[], evidence: string[] = []) {
  const handoffLines = [...handoffEvidenceCompletionLines(evidence), ...handoffCompletionLines(acceptanceCriteria)];
...
export function buildFirstDayDispatchCompletionTemplate(task: Pick<PersistedGatePlatformDispatchTask, "dispatchKey" | "acceptanceCriteria"> & Partial<Pick<PersistedGatePlatformDispatchTask, "dueLabel" | "title" | "evidence">>) {
  ...
  ].join("\n"), task.acceptanceCriteria, task.evidence);
  if (stepId === "first-draft") return withHandoffCompletion("第一章正文已生成并写回章节，钩子、冲突和章末追读已按首轮平台打法检查，可以进入审稿。", task.acceptanceCriteria, task.evidence);
  if (stepId === "first-review") return withHandoffCompletion("第一章审稿已完成，钩子、爽点、冲突、解释密度和章末追读问题已列出，可以进入二改。", task.acceptanceCriteria, task.evidence);
  if (stepId === "first-rewrite") return withHandoffCompletion("第一章二改或前三章改写已完成，审稿问题已逐项处理，并保留版本对照。", task.acceptanceCriteria, task.evidence);
  if (stepId === "publish-precheck") return withHandoffCompletion("平台包预检已完成，标题、简介、标签、卖点、样章和首轮数据回收口径已整理。", task.acceptanceCriteria, task.evidence);
  if (stepId === "story-support") return withHandoffCompletion("人物弧光、核心设定和平台土壤已补齐，后续正文生成可以直接引用。", task.acceptanceCriteria, task.evidence);
  if (stepId === "risk-recovery") return withHandoffCompletion("止损恢复条件已写清：入口卖点、前三章兑现或平台匹配度已至少改掉一项，并明确只验证一个变量。", task.acceptanceCriteria, task.evidence);
  if (stepId === "opening-hook") return withHandoffCompletion("第一章目标、钩子、冲突、转变和章末悬念已补齐，并按目标平台开头规则检查。", task.acceptanceCriteria, task.evidence);
  if (stepId === "skeleton") return withHandoffCompletion("作品骨架已完成，开头、结尾、主干、分支、叶片和土壤均已落地。", task.acceptanceCriteria, task.evidence);
  return task.acceptanceCriteria.length ? withHandoffCompletion(`首日派单已完成：${task.acceptanceCriteria.join("；")}。`, task.acceptanceCriteria, task.evidence) : "";
}
