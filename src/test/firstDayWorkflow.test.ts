...
test("buildFirstDayWorkflow", async (t) => {
  ...
  await t.test("injects project start tactics and model route recheck into first-day execution", () => {
    ...
    assert.ok(workflow.executionPackage.completionEvidenceTemplate.includes("成功率"));
    assert.ok(workflow.executionPackage.completionEvidenceTemplate.includes("质量分"));
    assert.ok(workflow.executionPackage.completionEvidenceTemplate.includes("失败样本"));
    assert.ok(workflow.executionPackage.completionEvidenceTemplate.includes("放量结论"));
