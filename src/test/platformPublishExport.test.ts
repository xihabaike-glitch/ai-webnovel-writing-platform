...
test("buildPlatformPublishExportCenter", async (t) => {
  ...
  await t.test("allows export when chapters and checklist pass final preflight", () => {
    ...
    assert.equal(center.executionHandoffs[0].actionKind, "save_publish_baseline");
    assert.equal(center.executionHandoffs[0].actionLabel, "保存发布基准");
    assert.equal(center.executionHandoffs[0].actionHref, "#package-version-history");
    assert.ok(center.executionHandoffs[0].currentAction.includes("先保存发布基准"));
    assert.equal(center.executionHandoffSummary.readyCount, 1);
    assert.equal(center.executionHandoffSummary.blockedCount, 0);
    assert.equal(center.executionHandoffSummary.primaryAction?.actionKind, "save_publish_baseline");
    ...
    assert.ok(center.executionHandoffSummary.headline.includes("先保存发布基准"));
    assert.equal(center.effectCaptureSummary.status, "needs_record");
