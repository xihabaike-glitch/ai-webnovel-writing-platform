...
test("project detail page renders the single-project acceptance sheet", () => {
  ...
  assert.ok(projectPage.includes("当前实跑动作"));
  assert.ok(projectPage.includes("item.runbookStep.title"));
  assert.ok(projectPage.includes("item.runbookStep.sampleAction"));
  assert.ok(projectPage.includes("item.runbookStep.proofToCapture"));
  assert.ok(projectPage.includes("item.runbookStep.rollbackIfWeak"));
...
test("project detail page carries gate return through control and batch panels", () => {
  ...
  assert.ok(controlPanel.includes("dashboard.productionClosure.map"));
  assert.ok(controlPanel.includes("closure.tone"));
  assert.ok(controlPanel.includes("closure.targetHref"));
